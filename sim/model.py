...
class Config:
    ...
    # trace-driven decision schedule (E10): replay production wake times
    # from a CSV (cohort, agent, t_offset_s) instead of Poisson clocks.
    wake_trace: str | None = None
    wake_cohort: str = "active"       # active | median
    ...
    def label(self) -> str:
        ...
        if self.bursty:
            lab += "+b"
        if self.wake_trace:
            lab += "+tr" + self.wake_cohort[0].upper()
        return lab
...
class Sim:
    def __init__(self, cfg: Config):
        ...
        if cfg.wake_trace:
            # replay production decision timestamps (one cohort of 10 agents)
            import csv as _csv
            with open(cfg.wake_trace) as f:
                for row in _csv.DictReader(f):
                    if row["cohort"] != cfg.wake_cohort:
                        continue
                    a = int(row["agent"]) % N
                    self._push(float(row["t_offset_s"]), "wake", a)
        else:
            for i in range(N):
    ...
    def _on_wake(self, i: int):
        ...
        if not cfg.wake_trace:                # trace mode: wakes pre-scheduled
            self._push(self._exp(cfg.wake_mean), "wake", i)
...
def run_once(cfg: Config) -> dict:
    ...
    return {
        ...
        "wake_cohort": cfg.wake_cohort if cfg.wake_trace else "",
